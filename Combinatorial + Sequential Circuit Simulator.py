...
def settle_combinational(blocks, wires, max_iters=256):
    for _ in range(max_iters):
        ...
        # compute non-memory blocks (inputs already updated)
        for bl in blocks:
            if isinstance(bl, M):
                continue
            prev = getattr(getattr(bl, "output", None), "signal", None)
            bl.compute()
            curr = getattr(getattr(bl, "output", None), "signal", None)
            if prev != curr:
                changed = True
...
while True:
    ...
    # ------------------------------------------------------------------
    for bl in blocks:
        if not isinstance(bl, M): 
             bl.compute()

    for w in wires:
        if w.end:
            w.end.signal = w.start.signal
        
    # ------------------------------------------------------------------
    # Sequential Logic (when SPACE pressed)
    # ------------------------------------------------------------------
    if clock_tick:
        ...
        # settle to push new inputs through combinational logic to M-OUT
        ...
        # settle again so the new M outputs propagate, ready for next cycle
        settle_combinational(blocks, wires)
